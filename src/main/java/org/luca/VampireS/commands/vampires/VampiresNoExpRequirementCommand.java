...
public class VampiresNoExpRequirementCommand extends CompositeCommand {

    public VampiresNoExpRequirementCommand(CompositeCommand parent) {
        super(parent, "expreq");
    }
    ...
    public void setup() {
        setOnlyPlayer(false);
        setPermission("vampires.command.expreq");
        setParametersHelp("commands.vampires.expreq.parameters");
        setDescription("commands.vampires.expreq.description");
    }
    ...
    public boolean execute(User user, String label, List<String> args) {
        ...
        switch(args.get(0)) {
            case "enable":
                ((VampireSPlugin) plugin).noExpReq().remove(target.getUniqueId());
                user.sendMessage(plugin, "commands.vampires.noexpreq.enable", VampireSPlugin.PREFIX, VampireSPlugin.PREFIX_VALUE,
                        "[name]", Objects.requireNonNull(target.getPlayer()).getDisplayName());
                break;
            case "disable":
                ((VampireSPlugin) plugin).noExpReq().add(target.getUniqueId());
                user.sendMessage(plugin, "commands.vampires.noexpreq.disable", VampireSPlugin.PREFIX, VampireSPlugin.PREFIX_VALUE,
                        "[name]", Objects.requireNonNull(target.getPlayer()).getDisplayName());
                break;
        }
